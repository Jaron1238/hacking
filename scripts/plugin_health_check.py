...
class PluginHealthChecker:
    ...
    def check_all_plugins(self) -> Dict[str, Any]:
        ...
        healthy_plugins = sum(
            1 for report in self.health_report.values() if report["status"] == "healthy"
        )
        warning_plugins = sum(
            1 for report in self.health_report.values() if report["status"] == "warning"
        )
        error_plugins = sum(
            1 for report in self.health_report.values() if report["status"] == "error"
        )
        ...
        self.health_report["_summary"] = {
            ...
            "health_percentage": (
                (healthy_plugins / total_plugins * 100) if total_plugins > 0 else 0
            ),
        }
    ...
    def _check_requirements_file(self, plugin_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"has_requirements": True, "dependencies": dependencies}
        except Exception:
            return {"has_requirements": False, "dependencies": []}
    ...
    def _check_plugin_functionality(self, plugin: BasePlugin) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"metadata_ok": metadata_ok, "run_method_works": run_method_ok}
        except Exception as e:
            return {"metadata_ok": False, "run_method_works": False, "error": str(e)}
