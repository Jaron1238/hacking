...
def download_oui_file():
    ...
    for source in OUI_SOURCES:
        ...
        try:
            ...
            with urllib.request.urlopen(req) as response, open(OUI_LOCAL_PATH, 'wb') as out_file:
                data = response.read()
                out_file.write(data)
            logger.info(f"OUI-Datei von {source['name']} erfolgreich nach '{OUI_LOCAL_PATH}' heruntergeladen.")
...
def build_oui_map() -> Dict[str, str]:
    ...
    if not OUI_LOCAL_PATH.exists():
        logger.warning(f"Lokale OUI-Datei ('{OUI_LOCAL_PATH}') nicht gefunden. Versuche Download...")
    ...
    if not mapping:
        logger.warning("Keine Einträge aus der OUI-Datei geladen. Hersteller-Lookup wird nicht funktionieren.")
...
def lookup_vendor(mac: Optional[str]) -> Optional[str]:
    ...
    if is_local_admin_mac(mac): return "(Lokal / Randomisiert)"
    pref = mac.replace("-", ":").upper()[:8]
    return OUI_MAP.get(pref)

def intelligent_vendor_lookup(mac: Optional[str], client_state: Optional["ClientState"] = None) -> Optional[str]:
    """
...
def ie_fingerprint_hash(ies: Dict[int, List[str]]) -> str:
    ...
    if key in _IE_FP_CACHE: return _IE_FP_CACHE[key]
...
def _parse_rsn_details(hex_data: str) -> Dict:
    details = {"pairwise_ciphers": set(), "akm_suites": set(), "mfp_capable": False, "mfp_required": False}
    try:
        data = bytes.fromhex(hex_data)
        offset = 4
        ...
        for _ in range(count):
            ...
            if suite == "000fac02": details["pairwise_ciphers"].add("TKIP")
            elif suite == "000fac04": details["pairwise_ciphers"].add("CCMP-128 (AES)")
            offset += 4
        count = int.from_bytes(data[offset:offset+2], 'little')
        offset += 2
        for _ in range(count):
            suite = data[offset:offset+4].hex()
            if suite == "000fac01": details["akm_suites"].add("802.1X (EAP)")
            elif suite == "000fac02": details["akm_suites"].add("PSK")
            offset += 4
        if len(data) > offset:
...
def parse_ies(ies_dict: Dict[int, List[str]], detailed: bool = False) -> Dict[str, any]:
    parsed = {"security": set(), "standards": set(), "vendor_specific": {}, "roaming_features": set()}
    if 48 in ies_dict:
    ...
    if 45 in ies_dict or 61 in ies_dict:
        ...
        if detailed and 45 in ies_dict: parsed["ht_caps"] = _parse_ht_capabilities(ies_dict[45][0])
    if 191 in ies_dict:
        parsed["standards"].add("802.11ac")
        if detailed and 191 in ies_dict: parsed["vht_caps"] = _parse_vht_capabilities(ies_dict[191][0])
    if 255 in ies_dict:
        for hex_data in ies_dict[255]:
            if hex_data.startswith('23'):
                parsed["standards"].add("802.11ax")
                if detailed: parsed["he_caps"] = {"present": True}
    if 55 in ies_dict: parsed["roaming_features"].add("802.11r")
    if 221 in ies_dict:
        for hex_data in ies_dict[221]:
            try:
                if hex_data.startswith("506f9a"):
                    if hex_data.startswith("506f9a09"): parsed["vendor_specific"]["Wi-Fi Direct"] = True
                    if hex_data.startswith("506f9a10"): parsed["roaming_features"].add("802.11v")
