...
class MetricsCollector:
    ...
    def _start_cleanup(self) -> None:
        ...
        self._cleanup_task = threading.Timer(self._cleanup_interval, self._cleanup_old_metrics)
    ...
    def record_gauge(self, name: str, value: Union[int, float], labels: Optional[Dict[str, str]] = None) -> None:
        """Recorde Gauge-Metrik."""
        series = self.create_metric(name, "gauge")
        series.add_point(value, labels)
    
    def record_counter(self, name: str, increment: Union[int, float] = 1, labels: Optional[Dict[str, str]] = None) -> None:
    ...
    def get_metric_summary(self, name: str, duration_seconds: Optional[float] = None) -> Optional[Dict[str, Any]]:
    ...
    def decorator(func: T) -> T:
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                if metric_type == "timing":
                    _global_metrics.record_timing(f"{operation_name}_error", duration, error_labels)
                elif metric_type == "counter":
                    _global_metrics.record_counter(f"{operation_name}_error", 1, error_labels)
...
class AnalysisMetrics:
    ...
    @staticmethod
    def record_clustering(algorithm: str, duration: float, num_clusters: int, num_points: int) -> None:
    ...
    def record_feature_extraction(duration: float, num_features: int) -> None:
        ...
        _global_metrics.record_timing("analysis_feature_extraction_duration", duration, labels)
...
class CaptureMetrics:
    ...
    def record_interface_status(interface: str, active: bool) -> None:
        ...
        _global_metrics.record_gauge("capture_interface_active", 1 if active else 0, labels)
...
class AsyncMetricsCollector:
    ...
    async def record_gauge(self, name: str, value: Union[int, float], labels: Optional[Dict[str, str]] = None) -> None:
        ...
        await loop.run_in_executor(None, self.collector.record_gauge, name, value, labels)
    
    async def record_counter(self, name: str, increment: Union[int, float] = 1, labels: Optional[Dict[str, str]] = None) -> None:
        """Async Counter-Recording."""
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, self.collector.record_counter, name, increment, labels)
    
    async def record_timing(self, name: str, duration: float, labels: Optional[Dict[str, str]] = None) -> None:
        """Async Timing-Recording."""
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, self.collector.record_timing, name, duration, labels)
    
    async def get_metric_summary(self, name: str, duration_seconds: Optional[float] = None) -> Optional[Dict[str, Any]]:
        """Async Metric-Summary."""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, self.collector.get_metric_summary, name, duration_seconds)
    
    async def export_metrics(self, format_type: str = "json") -> str:
        """Async Metrics-Export."""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, self.collector.export_metrics, format_type)


# Convenience-Funktionen
def record_timing(operation_name: str, duration: float, labels: Optional[Dict[str, str]] = None) -> None:
    """Recorde Timing-Metrik."""
    _global_metrics.record_timing(operation_name, duration, labels)


def record_counter(operation_name: str, increment: Union[int, float] = 1, labels: Optional[Dict[str, str]] = None) -> None:
    """Recorde Counter-Metrik."""
    _global_metrics.record_counter(operation_name, increment, labels)


def record_gauge(operation_name: str, value: Union[int, float], labels: Optional[Dict[str, str]] = None) -> None:
    """Recorde Gauge-Metrik."""
    _global_metrics.record_gauge(operation_name, value, labels)


def get_metric_summary(operation_name: str, duration_seconds: Optional[float] = None) -> Optional[Dict[str, Any]]:
    """Hole Metrik-Zusammenfassung."""
