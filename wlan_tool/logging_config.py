...
class PerformanceHandler(logging.Handler):
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        memory_usage = [
            m["memory_usage"] for m in self.performance_metrics if m["memory_usage"]
        ]
...
def log_with_context(
    logger: logging.Logger, level: int, message: str, context: Dict[str, Any], **kwargs
):
...
def get_performance_summary() -> Dict[str, Any]:
    ...
    valid_summaries = [
        s for s in summaries if "total_operations" in s and s["total_operations"] > 0
    ]
    if valid_summaries:
        combined["avg_duration"] = sum(
            s.get("avg_duration", 0) for s in valid_summaries
        ) / len(valid_summaries)
        combined["max_duration"] = max(
            s.get("max_duration", 0) for s in valid_summaries
        )
        combined["min_duration"] = min(
            s.get("min_duration", 0) for s in valid_summaries
        )
        combined["avg_memory_usage"] = sum(
            s.get("avg_memory_usage", 0) for s in valid_summaries
        ) / len(valid_summaries)
        combined["max_memory_usage"] = max(
            s.get("max_memory_usage", 0) for s in valid_summaries
        )
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                log_performance(
                    logger, operation_name or func.__name__, duration, memory_usage
                )
