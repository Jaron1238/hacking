...
class MemoryCache(Generic[T]):
    ...
    def __init__(
        self, max_size: int = 1000, default_ttl: float = Constants.CACHE_TTL_S
    ):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "hit_rate": hit_rate,
                "oldest_entry": (
                    min(entry.age for entry in self._cache.values())
                    if self._cache
                    else 0
                ),
                "newest_entry": (
                    max(entry.age for entry in self._cache.values())
                    if self._cache
                    else 0
                ),
            }
...
class FileCache(Generic[T]):
    ...
    def __init__(
        ...
        max_file_size: int = Constants.MAX_FILE_SIZE_MB * 1024 * 1024,
    ):
...
class HybridCache(Generic[T]):
    ...
    def __init__(
        self, memory_size: int = 1000, cache_dir: Optional[Union[str, Path]] = None
    ):
...
class AnalysisResultCache:
    ...
    def __init__(self, cache_dir: Optional[Union[str, Path]] = None):
        self.cache = HybridCache[Dict[str, Any]](memory_size=100, cache_dir=cache_dir)
    ...
    def set_client_features(
        self, client_mac: str, features: Dict[str, Any], ttl: float = 3600
    ) -> None:
    ...
    def set_clustering_result(
        self, state_hash: str, result: Dict[str, Any], ttl: float = 1800
    ) -> None:
