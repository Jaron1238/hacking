...
from .. import utils
from ..utils import is_local_admin_mac, is_valid_bssid, parse_ies
from .data_models import APState, ClientState, EventType, Welford
...
class WifiAnalysisState:
    ...
    def build_from_events(
        self, ev_iterator: Iterable[Dict], detailed_ies: bool = False
    ):
        ...
        for ev in ev_iterator:
            ...
            count += 1
        logger.info(
            "Zustand aus %d Events aufgebaut/aktualisiert: APs=%d, Clients=%d, SSIDs=%d",
            count,
            len(self.aps),
            len(self.clients),
            len(self.ssid_map),
    ...
    def update_from_event(self, ev: dict, detailed_ies: bool = False):
        ...
        if client_mac:
            if client_mac not in self.clients:
                self.clients[client_mac] = ClientState(
                    mac=client_mac,
                    first_seen=ts,
                    randomized=is_local_admin_mac(client_mac),
                )
        ...
        if ev_type in ("beacon", "probe_resp") and bssid:
            ...
            if ev_type == "beacon":
                ...
                if ssid:
                    self.ssid_map[ssid]["sources"]["beacon"].add(bssid)
            else:
                ap.probe_resp_count += 1
                if ssid:
                    self.ssid_map[ssid]["sources"]["probe_resp"].add(bssid)
            ...
            if client_mac in self.clients:
                self.clients[client_mac].hostname = ev.get("hostname")
    ...
    def prune_state(self, current_ts: float, threshold_s: int) -> int:
        stale_bssids = {
            ...
            if (current_ts - ap.last_seen) > threshold_s
        }
        if stale_bssids:
            for bssid in stale_bssids:
                del self.aps[bssid]
                if bssid in self.clients_seen_with_bssid:
                    del self.clients_seen_with_bssid[bssid]
        stale_clients = {
            mac
            for mac, client in self.clients.items()
            if (current_ts - client.last_seen) > threshold_s
        }
        if stale_clients:
        ...
        stale_seq_keys = {
            key
            for key in self.seq_local
            if key[0] in stale_clients or key[1] in stale_bssids
        }
        if stale_seq_keys:
        ...
        for ssid, sources_info in list(self.ssid_map.items()):
            ...
            for source_type, mac_set in list(sources_info["sources"].items()):
                ...
                if not mac_set:
                    del sources_info["sources"][source_type]
