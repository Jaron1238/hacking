...
class DBSCANClustering(BaseClusteringAlgorithm):
    ...
    def fit(self, X: np.ndarray) -> "DBSCANClustering":
        """Trainiere DBSCAN."""
        logger.info(
            f"Training DBSCAN with eps={self.eps}, min_samples={self.min_samples}"
        )
        ...
        logger.info(
            f"DBSCAN trained: {self.n_clusters_} clusters, {self.n_noise_} noise points"
        )
        return self
...
class ClusteringEvaluator:
    ...
    def compare_algorithms(
        self, X: np.ndarray, algorithms: Dict[str, BaseClusteringAlgorithm]
    ) -> pd.DataFrame:
        ...
        for name, algorithm in algorithms.items():
            try:
                ...
                logger.info(
                    f"Evaluated {name}: {metrics['n_clusters']} clusters, "
...
class ClusteringPipeline:
    ...
    def create_algorithm(
        self, algorithm_name: str, **kwargs
    ...
    def auto_cluster(
        self, X: np.ndarray, target_clusters: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        available_algorithms = {
            k: v for k, v in algorithm_configs.items() if k in self.algorithms
        }
        ...
        best_algorithm = comparison_df.loc[
            comparison_df["silhouette_score"].idxmax(), "algorithm"
        ]
...
def create_gaussian_mixture_clustering(
    n_components: int = 8,
