...
class ModelEvaluator:
    ...
    def evaluate_classification_model(
        ...
        try:
            ...
            record_timing(
                "ml_classification_evaluation_duration",
            ...
            logger.info(
                f"Classification evaluation completed: accuracy={accuracy:.4f}, f1={f1:.4f}"
            )
            return evaluation
    ...
    def evaluate_regression_model(
        ...
        try:
            ...
            # Metriken aufzeichnen
            record_timing(
                "ml_regression_evaluation_duration", evaluation["evaluation_duration"]
            )
            ...
            logger.info(
                f"Regression evaluation completed: r2={r2:.4f}, rmse={rmse:.4f}"
            )
            return evaluation
    ...
    def evaluate_clustering_model(
        ...
        try:
            ...
            # Metriken aufzeichnen
            record_timing(
                "ml_clustering_evaluation_duration", evaluation["evaluation_duration"]
            )
            ...
            logger.info(
                f"Clustering evaluation completed: silhouette={silhouette:.4f}, clusters={n_clusters}"
            )
            return evaluation
    ...
    def compare_models(self, evaluations: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        if not comparison_df.empty:
            ...
            if "accuracy" in comparison_df.columns:
                best_classification = comparison_df.loc[
                    comparison_df["accuracy"].idxmax()
                ]
                logger.info(
                    f"Best classification model: {best_classification['model_name']} "
            ...
            if "r2_score" in comparison_df.columns:
                ...
                logger.info(
                    f"Best regression model: {best_regression['model_name']} "
                    f"(r2: {best_regression['r2_score']:.4f})"
                )

            if "silhouette_score" in comparison_df.columns:
                best_clustering = comparison_df.loc[
                    comparison_df["silhouette_score"].idxmax()
                ]
                logger.info(
                    f"Best clustering model: {best_clustering['model_name']} "
    ...
    def generate_evaluation_report(
        self, evaluation: Dict[str, Any], save_plots: bool = True
    ) -> Dict[str, Any]:
    ...
    def _generate_evaluation_plots(self, evaluation: Dict[str, Any]) -> List[str]:
        ...
        try:
            if model_type == "classification":
                ...
                if (
                    "probabilities" in evaluation
                    and evaluation["probabilities"] is not None
                ):
                    plt.figure(figsize=(8, 6))
                ...
                if "predictions" in evaluation:
                    ...
                    plot_path = (
                        self.results_dir / f"{model_name}_actual_vs_predicted.png"
                    )
                    plt.savefig(plot_path, dpi=300, bbox_inches="tight")
                    plt.close()
                    plots.append(str(plot_path))
                ...
                if "cluster_info" in evaluation:
                    ...
                    plot_path = (
                        self.results_dir / f"{model_name}_cluster_distribution.png"
                    )
                    plt.savefig(plot_path, dpi=300, bbox_inches="tight")
                    plt.close()
                    plots.append(str(plot_path))
    ...
    def get_evaluation_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "evaluation_duration_avg": np.mean(
                [e.get("evaluation_duration", 0) for e in self.evaluation_history]
            ),
        }
