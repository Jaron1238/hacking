...
class MLModelTrainer:
    ...
    def prepare_training_data(
        ...
        logger.info(
            f"Prepared data: {X_scaled.shape[0]} samples, {X_scaled.shape[1]} features"
        )
        return X_scaled, y
    ...
    def _encode_categorical_features(
        self, X: np.ndarray, feature_columns: List[str]
    ) -> np.ndarray:
    ...
    def train_device_classifier(
        self, X: np.ndarray, y: np.ndarray, model_name: str = "device_classifier"
    ) -> Dict[str, Any]:
        ...
        logger.info(
            f"Training completed: {best_model_name} with {results[best_model_name]['accuracy']:.4f} accuracy"
        )

        return training_info

    def train_anomaly_detector(
        self, X: np.ndarray, model_name: str = "anomaly_detector"
    ) -> Dict[str, Any]:
        ...
        models = {
            ...
            "local_outlier_factor": LocalOutlierFactor(
                n_neighbors=20, contamination=0.1, n_jobs=-1
            ),
        ...
        for name, model in models.items():
            ...
            logger.info(
                f"{name}: {anomaly_count} anomalies detected ({anomaly_count/len(X)*100:.2f}%)"
            )

        # Bestes Modell auswählen (basierend auf Anomalie-Rate)
        best_model_name = min(
            results.keys(), key=lambda k: abs(results[k]["anomaly_rate"] - 0.1)
        )
        best_model = trained_models[best_model_name]
        ...
        # Metriken aufzeichnen
        record_timing(
            "ml_anomaly_training_duration", training_info["training_duration"]
        )
    ...
    def train_behavior_predictor(
        self, X: np.ndarray, y: np.ndarray, model_name: str = "behavior_predictor"
    ) -> Dict[str, Any]:
        ...
        # Metriken aufzeichnen
        record_timing(
            "ml_behavior_training_duration", training_info["training_duration"]
        )
    ...
    def _save_model(
        self, model: Any, model_name: str, training_info: Dict[str, Any]
    ) -> None:
...
class AutoMLPipeline:
    ...
    def run_automated_training(
        self, data: pd.DataFrame, task_type: str, target_column: Optional[str] = None
    ) -> Dict[str, Any]:
