...
from .models import BaseMLModel, DeviceClassifier, AnomalyDetector, BehaviorPredictor
...
class MLInferenceEngine:
    ...
    def load_clustering_model(self, model_name: str, algorithm_type: str) -> bool:
        ...
        try:
            from .clustering import (
                ...
                SpectralClustering, GaussianMixtureClustering
            )
    ...
    def predict_device_type(self, features: np.ndarray, model_name: str = "device_classifier") -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                'confidence': float(np.max(probabilities)) if probabilities is not None else 1.0,
            ...
            logger.debug(f"Device prediction: {prediction} (confidence: {result['confidence']:.3f})")
            return result
    ...
    def detect_anomaly(self, features: np.ndarray, model_name: str = "anomaly_detector") -> Dict[str, Any]:
        """Anomalie-Erkennung."""
        start_time = time.time()
    ...
    def predict_behavior(self, features: np.ndarray, model_name: str = "behavior_predictor") -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(model, 'predict_confidence'):
                prediction, confidence = model.predict_confidence(features.reshape(1, -1))
            ...
            logger.debug(f"Behavior prediction: {prediction} (confidence: {confidence:.3f})")
            return result
    ...
    def cluster_data(self, features: np.ndarray, model_name: str = "clustering_model") -> Dict[str, Any]:
        """Daten-Clustering."""
        start_time = time.time()
    ...
    def batch_predict(self, features_list: List[np.ndarray], prediction_type: str = "device") -> List[Dict[str, Any]]:
        """Batch-Vorhersagen für mehrere Samples."""
        logger.info(f"Batch prediction: {len(features_list)} samples, type: {prediction_type}")
    ...
    async def async_predict(self, features: np.ndarray, prediction_type: str = "device") -> Dict[str, Any]:
    ...
    def get_inference_stats(self) -> Dict[str, Any]:
        ...
        confidences = [entry.get('confidence', 0) for entry in self.inference_history if 'confidence' in entry]
...
def quick_device_classification(features: np.ndarray, model_name: str = "device_classifier") -> str:
...
def quick_anomaly_detection(features: np.ndarray, model_name: str = "anomaly_detector") -> bool:
...
def quick_behavior_prediction(features: np.ndarray, model_name: str = "behavior_predictor") -> float:
