...
from .exceptions import (
    ResourceError, FileSystemError, DatabaseError, 
    AnalysisError, NetworkError, HardwareError
)
...
class RecoveryManager:
    ...
    def register_strategy(self, exception_type: type, recovery_func: Callable, 
    ...
    def recover(self, exception: Exception, operation: Optional[str] = None) -> Any:
        ...
        if strategy['attempts'] >= strategy['max_attempts']:
            self.logger.error(f"Max recovery attempts exceeded for {exception_type.__name__}")
        ...
        try:
            ...
            self.logger.info(f"Attempting recovery for {exception_type.__name__} (attempt {strategy['attempts']})")
    ...
    # Recovery-Strategien
    def _recover_file_system_error(self, exception: FileSystemError, operation: Optional[str]) -> Any:
    ...
    def _recover_database_error(self, exception: DatabaseError, operation: Optional[str]) -> Any:
    ...
    def _recover_resource_error(self, exception: ResourceError, operation: Optional[str]) -> Any:
    ...
    def _recover_network_error(self, exception: NetworkError, operation: Optional[str]) -> Any:
        """Recovery für Network-Fehler."""
        # Warte und versuche erneut
        time.sleep(2.0)
        return True
    
    def _recover_hardware_error(self, exception: HardwareError, operation: Optional[str]) -> Any:
        ...
        try:
            ...
            if log_dir.exists():
                for log_file in log_dir.glob("*.log.*"):
                    try:
                        if log_file.stat().st_mtime < time.time() - 86400:  # Älter als 1 Tag
                            log_file.unlink()
                    except Exception:
                        pass
...
def with_recovery(operation_name: Optional[str] = None, fallback_func: Optional[Callable] = None):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            try:
                ...
                if fallback_func:
                    recovery_manager.register_fallback(operation_name or func.__name__, fallback_func)
