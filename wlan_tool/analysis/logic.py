...
from ..storage.data_models import APState, ClientState, InferenceResult
from ..storage.state import WifiAnalysisState
from ..utils import ie_fingerprint_hash, intelligent_vendor_lookup, lookup_vendor
...
def features_for_pair_basic(
    ssid: str, bssid: str, state: WifiAnalysisState
) -> Tuple[Dict, List[str]]:
    ...
    bi_mc = (
        (ap.beacon_intervals.most_common(1)[0])
        if ap and ap.beacon_intervals
        else (0, 0)
    )
    seq_support = sum(
        state.seq_local.get((c, bssid), {}).get("monotonic_count", 0)
        for c in supporting_clients
    )
...
def heuristic_score(feat: dict, norm: Optional[Dict] = None) -> float:
    ...
    scores = {
        ...
        "probe": min(
            1.0, feat.get("probe_resp_count", 0) / (norm["probe_resp_count"] + 1e-6)
        ),
        "client": min(
            1.0, feat.get("supporting_clients", 0) / (norm["supporting_clients"] + 1e-6)
        ),
        "rssi": max(
            0.0, 1.0 - (feat.get("rssi_std", 0.0) / (norm["rssi_std_max"] + 1e-6))
        ),
        "channel": 1.0,
...
def score_pairs_with_recency_and_matching(
    state: WifiAnalysisState, model=None
) -> List[InferenceResult]:
    pair_components = {
        (s, b): features_for_pair_basic(s, b, state)
        for s, info in state.ssid_map.items()
        if s and s != "<hidden>"
    ...
    dynamic_norm = {
        ...
        "supporting_clients": max(
            1.0, df_features["supporting_clients"].quantile(0.90)
        ),
    ...
    pair_scores = {
        k: heuristic_score(feat, norm=dynamic_norm)
        for k, (feat, _) in pair_components.items()
    }
    if model is not None and not df_features.empty:
        ...
        try:
            ...
            for i, (key, score) in enumerate(pair_scores.items()):
                ml_score = ml_probs[i]
                combined_score = (ml_score * config.ML_SCORE_WEIGHT) + (
                    score * config.HEURISTIC_SCORE_WEIGHT
                )
                pair_scores[key] = combined_score
        except Exception as e:
            logger.warning(
                f"ML-Modell-Anwendung fehlgeschlagen: {e}. Verwende nur heuristische Scores."
            )
    ie_groups = defaultdict(set)
    for b, ap in state.aps.items():
        if h := ie_fingerprint_hash(ap.ies):
            ie_groups[h].add(b)
    multi_ssid_bssids = {
        b
        for bset in ie_groups.values()
        if len({s for b in bset for s, i in state.ssid_map.items() if b in i["bssids"]})
        >= config.MULTI_SSID_THRESHOLD
        for b in bset
    }
    G = nx.Graph()
    ...
    for u, v in matching:
        if u in ssids:
            assignment[u] = (v, G[u][v]["weight"])
        else:
            assignment[v] = (u, G[u][v]["weight"])
    results: List[InferenceResult] = []
    for s, (b, score) in assignment.items():
        feat, sup = pair_components[(s, b)]
        label = (
            "high"
            if score >= config.SCORE_LABEL_HIGH
            else ("medium" if score >= config.SCORE_LABEL_MEDIUM else "low")
        )
        result = InferenceResult(
            ...
            components=feat,
            evidence={
                "supporting_clients": sup,
                "multi_ssid_allowed": (b in multi_ssid_bssids),
            },
        )
...
def features_for_client(client_state: ClientState) -> Optional[Dict[str, any]]:
    ...
    with ErrorContext("client_features_extraction", "CLIENT_FEATURES_ERROR"):
        ...
        try:
            ...
            return {
                "vendor": intelligent_vendor_lookup(client_state.mac, client_state)
                or "Unknown",
                ...
                "probe_count": (
                    len(client_state.probes) if hasattr(client_state, "probes") else 0
                ),
                "seen_with_ap_count": (
                    len(client_state.seen_with)
                    if hasattr(client_state, "seen_with")
                    else 0
                ),
                "was_in_powersave": (
                    1 if getattr(client_state, "last_powersave_ts", 0) > 0 else 0
                ),
                "is_randomized_mac": (
                    1 if getattr(client_state, "randomized", False) else 0
                ),
                ...
                "has_apple_ie": (
                    1 if parsed.get("vendor_specific", {}).get("Apple") else 0
                ),
                "has_ms_ie": (
                    1 if parsed.get("vendor_specific", {}).get("Microsoft") else 0
                ),
            }
        except Exception as e:
            raise AnalysisError(
                ...
                details={
                    "mac": getattr(client_state, "mac", "unknown"),
...
def features_for_client_behavior(client_state: ClientState) -> Optional[Dict[str, any]]:
    ...
    if np is None:
        logger.error("Numpy wird für die Verhaltensanalyse benötigt.")
        return None
    if len(client_state.all_packet_ts) < 5:
        return None
    ...
    sessions = []
    if intervals.any():
        session_breaks = np.where(intervals > 10.0)[
            0
        ]  # Session-Ende nach 10s Inaktivität
    ...
    avg_packets_per_session = (
        np.mean([s["packet_count"] for s in sessions]) if sessions else 0
    )
    ...
    if client_state.mcs_rates:
        ...
        if total_mcs_packets > 0:
            avg_mcs = (
                sum(k * v for k, v in client_state.mcs_rates.items())
                / total_mcs_packets
            )
    ...
    avg_snr = (
        client_state.rssi_w.mean - client_state.noise_w.mean
        if client_state.noise_w.n > 0
        else 0
    )
    fcs_error_rate = (
        client_state.fcs_error_count / client_state.count
        if client_state.count > 0
        else 0
    )
    ...
    return {
        ...
        "data_to_mgmt_ratio": data_to_mgmt_ratio,
        "rssi_std": rssi_std,
        "power_save_rate": power_save_rate,
        "unique_probes_count": len(client_state.probes),
        "unique_aps_seen_count": len(client_state.seen_with),
        "avg_mcs_rate": avg_mcs,
        "max_mcs_rate": max_mcs,
...
def prepare_client_feature_dataframe(
    state: WifiAnalysisState, correlate_macs: bool = True
    ...
    if correlate_macs:
        ...
        logger.info(
            "MAC-Korrelation deaktiviert. Extrahiere Features für jede MAC-Adresse einzeln."
        )
        for mac, client_state in state.clients.items():
            if features := get_combined_features(client_state):
                features["mac"] = mac
...
def find_optimal_k_elbow_and_silhouette(
    X_scaled, max_k: Optional[int] = None
) -> Optional[int]:
    ...
    if max_k < 2:
        ...
        return None
    logger.info(
        "Suche optimales k für Clustering (2 bis %d) mit kombinierter Methode...", max_k
    )
    ...
    try:
        ...
        best_k_elbow = None
    logger.debug(
        f"Bestes k (Silhouette): {best_k_silhouette}, Bestes k (Ellenbogen): {best_k_elbow}"
    )
    ...
    with ErrorContext("client_clustering", "CLIENT_CLUSTERING_ERROR"):
        ...
        try:
            ...
            df_numeric = pd.get_dummies(
                df.drop(columns=["mac", "original_macs", "vendor"])
            )
            ...
            if use_encoder_path and ClientAutoencoder:
                logger.info(
                    "Autoencoder wird verwendet. Manuelle Feature-Gewichtung wird übersprungen."
                )
                if torch and Path(use_encoder_path).exists():
                    try:
                        embedding_dim = 16
                        encoder = ClientAutoencoder(
                            input_dim=X_scaled.shape[1], embedding_dim=embedding_dim
                        ).encoder
                        ...
                        logger.info(
                            f"Client-Features in {embedding_dim}-dimensionale Embeddings umgewandelt."
                        )
                    except Exception as e:
                        logger.error(
                            f"Fehler beim Laden/Anwenden des Encoders: {e}. Fallback auf Standard."
                        )
                else:
                    logger.warning(
                        f"Encoder-Modell nicht gefunden oder torch nicht verfügbar. Fallback auf Standard."
                    )
            else:
                logger.info(
                    "Wende manuelle Feature-Gewichtungen für Standard-Clustering an..."
                )
                X_scaled_df = pd.DataFrame(X_scaled, columns=df_numeric.columns)
                weights = config.CLIENT_CLUSTERING_FEATURE_WEIGHTS
                for feature, weight in weights.items():
                    cols_to_weight = [
                        col for col in X_scaled_df.columns if col.startswith(feature)
                    ]
                    if cols_to_weight:
                        logger.debug(
                            f"Gewichte Feature '{feature}' mit Faktor {weight}."
                        )
            ...
            if algo == "dbscan":
                ...
                logger.info(
                    f"DBSCAN fand {n_clusters_found} Cluster und {np.sum(labels == -1)} Ausreißer."
                )
            else:
...
def profile_clusters(feature_df: pd.DataFrame, clustered_df: pd.DataFrame) -> Dict:
    ...
    full_df = pd.merge(
        feature_df, clustered_df[["original_macs", "cluster"]], on="original_macs"
    )
    profiles = {}
    ...
    for cluster_id in sorted(full_df["cluster"].unique()):
        if cluster_id == -1:
            profiles[-1] = {
                "count": int(np.sum(full_df["cluster"] == -1)),
                "description": "Ausreißer / Rauschen",
            }
            continue
...
def features_for_ap(ap_state: APState) -> Optional[Dict[str, any]]:
    if not ap_state:
        return None
    bi_mc = (
        (ap_state.beacon_intervals.most_common(1)[0])
        if ap_state.beacon_intervals
        else (0, 0)
    )
    ...
    return {
        "ssid": ap_state.ssid or "<unknown>",
        "vendor": lookup_vendor(ap_state.bssid) or "Unknown",
        "channel": ap_state.channel or 0,
        "beacon_interval_mode": bi_mc[0],
        "beacon_interval_consistency": bi_mc[1] / (ap_state.beacon_count or 1),
        ...
        "supports_11k": 1 if "802.11k" in roaming else 0,
        "supports_11v": 1 if "802.11v" in roaming else 0,
...
def cluster_aps(
    ...
    if KMeans is None:
        ...
        return None
    rows = [
        dict(bssid=bssid, **(feat or {}))
        for bssid, ap in state.aps.items()
        if (feat := features_for_ap(ap))
    ]
    if not rows:
        logger.warning("Keine APs mit ausreichenden Merkmalen zum Clustern.")
        return None
...
def profile_ap_clusters(clustered_ap_df: pd.DataFrame) -> Dict:
    ...
    bool_cols = [
        col for col in clustered_ap_df.columns if clustered_ap_df[col].dtype == "bool"
    ]
...
def correlate_randomized_clients(state: WifiAnalysisState) -> Dict[str, List[str]]:
    ...
    randomized_clients = [
        c for c in state.clients.values() if c.randomized and c.probes
    ]
    ...
    correlated_groups = {
        fp: macs for fp, macs in probe_fingerprints.items() if len(macs) > 1
    }
    logger.info(
        "Fand %d Gruppen von potenziell zusammengehörigen randomisierten MACs.",
...
def _build_export_graph(
    ...
    for _, row in clustered_ap_df.iterrows():
        ...
        if bssid_str not in aps_to_export:
            continue
        ...
        roaming_support = bool(
            row.get("supports_11k")
            or row.get("supports_11v")
            or row.get("supports_11r")
        )
    ...
    if include_clients:
        ...
        if clustered_client_df is not None and not clustered_client_df.empty:
            mac_to_cluster = {
                ...
                for mac in str(row.get("original_macs", "")).split(", ")
                if mac
            }
        ...
        for mac, client_state in clients_to_export.items():
            ...
            for bssid in client_state.seen_with:
                if G.has_node(mac) and G.has_node(bssid):
                    rssi_dbm = float(
                        client_state.rssi_w.mean if client_state.rssi_w.n > 0 else -100
                    )
                    positive_weight = max(0.1, 100 + rssi_dbm)
                    # Add ISO timestamps for edge timeline animation
                    edge_start_iso = datetime.fromtimestamp(
                        client_state.first_seen
                    ).isoformat()
                    edge_end_iso = datetime.fromtimestamp(
                        client_state.last_seen
                    ).isoformat()
                    G.add_edge(
                        mac,
                        bssid,
...
def write_gexf13_enhanced(G: nx.Graph, out_file: str):
    ...
    if node_attrs:
        attributes_nodes = ET.SubElement(
        ...
        for idx, (title, attr_type) in enumerate(node_attrs.items()):
            ET.SubElement(
                attributes_nodes,
                "attribute",
                {"id": str(idx), "title": title, "type": attr_type},
            )

    if edge_attrs:
        attributes_edges = ET.SubElement(
            graph, "attributes", {"class": "edge", "mode": "static"}
        )
    ...
    for n_id, attrs in G.nodes(data=True):
        node_el = ET.SubElement(
            nodes, "node", {"id": str(n_id), "label": str(attrs.get("label", n_id))}
        )
        if "start" in attrs:
            node_el.set("start", str(attrs["start"]))
        if "end" in attrs:
            node_el.set("end", str(attrs["end"]))
        ...
        for idx, (title, _) in enumerate(node_attrs.items()):
            if title in attrs:
                ...
                ET.SubElement(
                    attvalues_el, "attvalue", {"for": str(idx), "value": str(value)}
                )
    ...
    for i, (u, v, attrs) in enumerate(G.edges(data=True)):
        ...
        if "kind" in attrs:
            edge_el.set("label", str(attrs["kind"]))
        if "start" in attrs:
            edge_el.set("start", str(attrs["start"]))
        if "end" in attrs:
            edge_el.set("end", str(attrs["end"]))
...
def export_ap_graph(
    ...
    try:
        ...
        if out_format.lower() == "graphml":
            ...
            logger.info(
                f"GraphML erfolgreich nach '{out_file}' exportiert. Knoten: {graph_to_export.number_of_nodes()}, Kanten: {graph_to_export.number_of_edges()}."
            )
        else:
