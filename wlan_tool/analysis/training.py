...
def train_model_from_confirmed_csv(confirmed_csv: str, feature_extractor: Callable, out_model: Optional[str] = None) -> Optional[object]:
    if pd is None: raise RuntimeError("pandas/scikit-learn nicht installiert")
    ...
    if not rows: logger.error("Keine gültigen Zeilen für das Training."); return None
    ...
    if out_model and joblib:
        try: joblib.dump(model, out_model); logger.info("Modell gespeichert in %s", out_model)
        except Exception as e: logger.exception("Fehler beim Speichern des Modells: %s", e)

    try:
        probs = model.predict_proba(X_test)[:, 1]
        auc, brier = roc_auc_score(y_test, probs), brier_score_loss(y_test, probs)
        logger.info(f"Modell-Evaluierung - Test AUC: {auc:.4f}, Brier Score: {brier:.4f}")
    except Exception as e: logger.debug("Modell-Evaluierungsmetriken fehlgeschlagen: %s", e)
    return model


def auto_retrain_if_enough(label_db: str, min_confirmed: int, out_model: str, feature_extractor: Callable):
    with tempfile.NamedTemporaryFile(prefix="labels_", suffix=".csv", delete=False, mode='w') as f:
        tmpf_name = f.name
    try:
        n = database.export_confirmed_to_csv(label_db, tmpf_name)
        if n >= min_confirmed:
            logger.info("Genug Labels (%d >= %d). Training wird gestartet...", n, min_confirmed)
            return train_model_from_confirmed_csv(tmpf_name, feature_extractor, out_model=out_model)
        else:
            logger.info("Nicht genug Labels (%d < %d). Überspringe.", n, min_confirmed)
            return None
    finally:
        if os.path.exists(tmpf_name): os.unlink(tmpf_name)

def train_client_autoencoder(state: WifiAnalysisState, out_model_path: str, embedding_dim: int = 16, epochs: int = 50) -> bool:
    """
    ...
    if pd is None or torch is None or ClientAutoencoder is None:
        logger.error("torch, pandas oder scikit-learn nicht installiert. Autoencoder-Training abgebrochen.")
        return False
    ...
    criterion = nn.MSELoss() # Mean Squared Error ist ein guter Loss für die Rekonstruktion
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 10 == 0:
            logger.info(f"Epoche [{epoch+1}/{epochs}], Rekonstruktions-Loss: {loss.item():.6f}")
    ...
    try:
        ...
        logger.info(f"Trainierter Client-Encoder erfolgreich gespeichert in: {out_model_path}")
        return True
    except Exception as e:
        logger.error(f"Fehler beim Speichern des Encoder-Modells: {e}")
        return False

def train_behavioral_model(event_db: str, label_db: str, out_model: str) -> Optional[object]:
    ...
    with database.db_conn_ctx(label_db) as conn:
        try:
            labeled_clients = pd.read_sql_query("SELECT mac, device_type FROM client_labels", conn)
        except pd.io.sql.DatabaseError:
            labeled_clients = pd.DataFrame()
            
    if labeled_clients.empty:
        logger.error("Keine gelabelten Clients in der Label-Datenbank für das Training gefunden.")
        return None
    ...
    if not feature_rows:
        logger.error("Konnte keine Verhaltens-Features für die gelabelten, im Scan gefundenen Clients extrahieren.")
        logger.error("Stellen Sie sicher, dass Ihre events.db und labels.db synchron sind und genügend Aktivität erfasst wurde.")
        return None
    ...
    training_data = pd.merge(features_df, labeled_clients, on='mac_lower', suffixes=('_features', '_labels'))
    
    if training_data.empty:
        logger.error("Trainingsdaten sind nach dem Zusammenführen leer.")
        return None

    target_column = 'device_type_labels' if 'device_type_labels' in training_data.columns else 'device_type'
    y = training_data[target_column]

    non_feature_columns = [col for col in training_data.columns if '_labels' in col or '_features' in col or 'mac' in col]
    non_feature_columns.append('device_type')

    X = training_data.drop(columns=non_feature_columns, errors='ignore')
    
    if X.empty:
        logger.error("Keine Feature-Spalten nach der Bereinigung übrig. Training nicht möglich.")
        return None
    ...
    if single_member_classes:
        logger.warning(f"Die folgenden Klassen haben nur 1 Mitglied und werden dem Trainingsdatensatz fest zugewiesen: {single_member_classes}")
    ...
    if not X_regulars.empty and len(X_regulars) >= min_samples_for_split:
        ...
        if not can_stratify:
            logger.warning(f"Stratifizierung nicht möglich (kleinste Klasse hat {min_class_count} Samples, benötigt >= 4). Verwende zufälligen Split.")
        
        X_train_reg, X_test, y_train_reg, y_test = train_test_split(
            X_regulars, y_regulars, test_size=test_size, random_state=42, stratify=stratify_option
        )
        ...
        logger.warning(f"Zu wenig Daten für Test/Train-Split (nur {total_samples} Samples, benötigt >= {min_samples_for_split}). Alle Daten werden fürs Training verwendet.")
        logger.warning("Für zuverlässige Modelle werden mindestens 8-10 Samples pro Gerätetyp empfohlen.")
        X_train = pd.concat([X_regulars, X_singles]) if not X_regulars.empty else X_singles
        y_train = pd.concat([y_regulars, y_singles]) if not y_regulars.empty else y_singles
        X_test = pd.DataFrame()
        y_test = pd.Series(dtype='object')

    logger.info(f"Trainingsdatensatz-Größe: {len(X_train)}, Testdatensatz-Größe: {len(X_test)}")
