...
from scapy.all import Dot11, Dot11Elt, Dot11Beacon, RadioTap, wrpcap, PcapReader
...
def _extract_seq(dot11) -> Optional[int]:
    try: return int(getattr(dot11, "SC", 0)) >> 4
        ...
        while current_layer:
            ...
            if isinstance(current_layer, Dot11Elt):
                if hasattr(current_layer, 'ID'):
                    info_bytes = bytes(current_layer.info) if hasattr(current_layer, 'info') else b""
...
def packet_to_event(pkt) -> Optional[WifiEvent]:
    ...
    if not pkt.haslayer(Dot11): return None
    ...
    if RadioTap and pkt.haslayer(RadioTap):
        ...
        if freq is not None:
            freq = int(freq)
            if 2412 <= freq <= 2484: rt_fields['channel'] = (freq - 2407) // 5
            elif 5180 <= freq <= 5825: rt_fields['channel'] = (freq - 5000) // 5
    ...
    if dot.type == 0: # Management Frame
        ...
        if dot.subtype in (8, 5): # Beacon or Probe Response
            ...
            try:
                ssid = pkt.info.decode(errors="ignore") if hasattr(pkt, 'info') else "<hidden>"
            ...
            probes = [bytes.fromhex(h).decode(errors='ignore') or "<broadcast>" for h in ies.get(0, [])]
            ies['probes'] = probes
            return event
        ...
        if pkt.haslayer(DNSQR) and pkt.dport == 53:
            try: event['dns_query'] = pkt[DNSQR].qname.decode(errors='ignore')
            except Exception: pass
        elif pkt.haslayer(DHCP):
            try:
                dhcp_options = {opt[0]: opt[1] for opt in pkt[DHCP].options if isinstance(opt, tuple)}
...
class ChannelHopper(threading.Thread):
    def __init__(self, iface: str, channels: List[int], sleep_interval: float):
        ...
        logger.info("Channel Hopper initialisiert für %s mit %d Kanälen.", iface, len(channels))

    def run(self):
        idx = 0
        if not self.channels:
            logger.warning("Channel Hopper hat keine Kanäle zum Springen und wird beendet.")
            return
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                try:
                    logger.debug(f"Setzen des Kanals mit Bandbreite {bw} fehlgeschlagen, versuche Fallback...")
                    fallback_command = ['iw', 'dev', self.iface, 'set', 'channel', str(channel)]
                    ...
                    logger.error("Fehler beim Setzen des Kanals auf %s: %s", self.iface, e_fallback)
                    time.sleep(5)
...
def packet_parser_worker(pcap_filename_queue: mp.Queue, db_queue: mp.Queue, live_queue: Optional[mp.Queue]):
    while True:
        try:
...
def packet_reader_thread(tcpdump_proc, pcap_filename_queue: mp.Queue, pcap_buff, stop_event, pcap_out):
    try:
        pcap_header = tcpdump_proc.stdout.read(24)
        if not pcap_header:
            logger.warning("tcpdump hat keinen Pcap-Header gesendet. Beende Reader-Thread.")
            return
        if pcap_out: pcap_buff.append(pcap_header)
        chunk_size = 1024 * 256
        while not stop_event.is_set():
            pkt_data_chunk = tcpdump_proc.stdout.read(chunk_size)
            if not pkt_data_chunk: break
            with tempfile.NamedTemporaryFile(delete=False, suffix='.pcap', prefix='capture_chunk_') as tmp_file:
                tmp_file.write(pcap_header)
                tmp_file.write(pkt_data_chunk)
                tmp_filename = tmp_file.name
            pcap_filename_queue.put(tmp_filename)
            if pcap_out: pcap_buff.append(pkt_data_chunk)
    except Exception as e:
...
def sniff_with_writer(
    ...
    if PcapReader is None: raise RuntimeError("scapy nicht installiert")
    
    pcap_filename_queue = mp.Queue(maxsize=128)
    db_event_queue: mp.Queue = mp.Queue(maxsize=20000)
    num_workers = max(1, mp.cpu_count() - 1)
    workers = [mp.Process(target=packet_parser_worker, args=(pcap_filename_queue, db_event_queue, live_queue), daemon=True) for _ in range(num_workers)]
    for w in workers: w.start()
    
    writer = database.BatchedEventWriter(db_path, db_event_queue, config.DB_BATCH_SIZE, config.DB_FLUSH_INTERVAL_S)
    
    channels_to_use = channels_override if channels_override is not None else config.CHANNELS_TO_HOP
    ...
    logger.info(f"Verwende BPF-Filter: '{bpf_filter}' zur Erfassung von Management- und Daten-Paketen.")
    ...
    reader = threading.Thread(target=packet_reader_thread, args=(tcpdump_proc, pcap_filename_queue, pcap_buff_bytes, stop_event, pcap_out), daemon=True, name="TcpdumpReader")
    threads.append(reader)
    stderr_drainer = threading.Thread(target=stderr_drainer_thread, args=(tcpdump_proc, stop_event), daemon=True, name="StderrDrainer")
    threads.append(stderr_drainer)
    
    for t in threads: t.start()
    
    logger.info("Sniffing auf %s für %ds mit %d Workern...", iface, duration, num_workers)
    ...
    try:
        ...
        except subprocess.TimeoutExpired: tcpdump_proc.kill()
        ...
        def _monitor_progress(stop_event_progress):
            with tqdm(total=max(1, initial_raw_size), desc="1. Pakete parsen ", unit="file") as pbar_parse, \
                 tqdm(total=max(1, initial_event_size), desc="2. Events schreiben", unit="evt") as pbar_write:
                while not stop_event_progress.is_set():
                    current_parsed = max(0, initial_raw_size - pcap_filename_queue.qsize())
                    pbar_parse.n = min(current_parsed, initial_raw_size)
                    pbar_parse.refresh()
                    current_written = max(0, initial_event_size - db_event_queue.qsize())
                    pbar_write.n = min(current_written, initial_event_size)
                    pbar_write.refresh()
                    if pcap_filename_queue.qsize() == 0 and db_event_queue.qsize() == 0 and pbar_parse.n >= initial_raw_size:
                        break
                    time.sleep(0.1)
                pbar_parse.n = initial_raw_size; pbar_parse.refresh()
                pbar_write.n = initial_event_size; pbar_write.refresh()

        progress_stop_event = threading.Event()
        progress_thread = threading.Thread(target=_monitor_progress, args=(progress_stop_event,), daemon=True)
        progress_thread.start()
        
        for _ in range(num_workers): pcap_filename_queue.put(None)
        for w in workers: w.join(timeout=20)
        if w.is_alive(): w.terminate()
        for t in [writer, hopper]: t.stop()
        ...
        if pcap_out and pcap_buff_bytes:
            try:
                with open(pcap_out, 'wb') as f:
                    for chunk in pcap_buff_bytes: f.write(chunk)
