...
class ErrorMessages:
    ...
    DISK_SPACE_INSUFFICIENT = (
        "Insufficient disk space: {required}MB required, {available}MB available"
    )
    ...
    ANALYSIS_INSUFFICIENT_DATA = (
        "Insufficient data for analysis: {required} required, {available} available"
    )
    ...
    # Resource Messages
    RESOURCE_MEMORY_INSUFFICIENT = (
        "Insufficient memory: {required}MB required, {available}MB available"
    )
...
def get_error_message(error_code: ErrorCodes, **kwargs) -> str:
    ...
    message_template = getattr(
        ErrorMessages, error_code.value, "Unknown error: {error_code}"
    )
    try:
