...
from .exceptions import ValidationError, ResourceError, HardwareError
...
class SystemResourceChecker(HealthChecker):
    ...
    async def _check_impl(self) -> HealthCheckResult:
        ...
        try:
            ...
            if cpu_percent > self.max_cpu_percent or memory.percent > self.max_memory_percent:
                status = HealthStatus.UNHEALTHY
                message = f"High resource usage: CPU {cpu_percent:.1f}%, Memory {memory.percent:.1f}%"
            elif cpu_percent > self.max_cpu_percent * 0.8 or memory.percent > self.max_memory_percent * 0.8:
    ...
    async def _check_impl(self) -> HealthCheckResult:
        ...
        try:
            ...
            if free_space_gb < self.min_free_space_gb:
                issues.append(f"Low disk space: {free_space_gb:.2f}GB (min: {self.min_free_space_gb}GB)")
...
class NetworkHealthChecker(HealthChecker):
    ...
    def __init__(self, test_urls: List[str] = None):
        ...
        self.test_urls = test_urls or ["https://www.google.com", "https://www.cloudflare.com"]
