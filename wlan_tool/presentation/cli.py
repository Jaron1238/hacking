...
def print_client_cluster_results(args, state, console):
    ...
    if use_correlation:
        ...
        table.add_column("Cluster ID", style="bold yellow")
        table.add_column("Zugehörige MACs (bei Gruppen)")
    else:
    ...
    for _, row in clustered_df.head(30).iterrows():
        ...
        if use_correlation:
            ...
            table.add_row(
                mac_str, row["vendor"], str(row["cluster"]), original_macs_str
            )
        else:
    ...
    for cid, profile in profiles.items():
        ...
        for feature in profile.get("top_features", []):
            deviation = profile.get("relative_deviations", {}).get(
                f"{feature}_rel_diff_pct", 0
            )
            console.print(
                f"    - {feature}: {profile.get(feature, 0):.2f} ([bold {'green' if deviation > 0 else 'red'}]{deviation:+.1f}%[/bold])"
            )
...
def print_ap_cluster_results(args, state, console):
    ...
    table.add_column("Roaming")
    table.add_column("Cluster ID", style="bold yellow")
    for _, row in clustered_df.iterrows():
        auth_str = "ENT" if row.get("is_enterprise_auth") else "PSK"
        roaming_parts = [
            r[9:]
            for r in ["supports_11k", "supports_11v", "supports_11r"]
            if row.get(r)
        ]
    ...
    for cid, profile in profiles.items():
        ...
        vendor_str = ", ".join(
            [f"{vendor} ({count})" for vendor, count in profile["vendors"].items()]
        )
        console.print(f"  - Hersteller: {vendor_str}")
        channel_str = ", ".join(
            [f"Kanal {ch} ({count})" for ch, count in profile["channels"].items()]
        )
        console.print(f"  - Kanäle: {channel_str}")
        console.print(f"  - Enterprise-Auth: {profile['enterprise_auth_pct']:.1f}%")
        console.print(
            f"  - Roaming-Support (11k/v/r): {profile['roaming_support_pct']:.1f}%"
        )
    return clustered_df
...
def interactive_label_ui(
    db_path_events: str, label_db_path: str, model_path: str, console
):
    ...
    model = (
        joblib.load(model_path) if model_path and Path(model_path).exists() else None
    )
    ...
    candidates = [
        c
        for c in results
        if config.UI_DEFAULT_MIN_SCORE <= c.score <= config.UI_DEFAULT_MAX_SCORE
    ][: config.UI_MAX_CANDIDATES]
    if not candidates:
    ...
    with database.db_conn_ctx(label_db_path) as lab_conn:
        console.print(
            f"[cyan]Starte interaktive Labeling-UI: {len(candidates)} Kandidaten gefunden.[/cyan]"
        )
        for res in candidates:
            ...
            ans = Prompt.ask(
                "Label (y=bestätigen / n=ablehnen / s=überspringen / q=beenden)",
                choices=["y", "n", "s", "q"],
                default="s",
            ).lower()
            if ans == "q":
...
def interactive_client_label_ui(label_db_path: str, state_obj, console):
    ...
    with database.db_conn_ctx(label_db_path) as conn:
        ...
        sorted_clients = sorted(
            state_obj.clients.items(), key=lambda item: item[1].last_seen, reverse=True
        )
        for mac, client in sorted_clients:
            vendor = utils.intelligent_vendor_lookup(mac, client) or "N/A"
            last_seen_str = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(client.last_seen)
            )
            probes_str = (
                ", ".join(sorted(list(client.probes))) if client.probes else "Keine"
            )
            console.print("\n" + "=" * 80)
            ...
            try:
                ...
                if new_label:
                    confirm = Prompt.ask(
                        f"Neuen Label '[bold]{new_label}[/bold]' hinzufügen und verwenden?",
                        choices=["y", "n"],
                        default="y",
                    ).lower()
                    if confirm == "y":
            ...
            if selected_type:
                try:
                    cursor.execute(
                        "INSERT OR REPLACE INTO client_labels (mac, device_type, source, ts) VALUES (?, ?, ?, ?)",
                    ...
                    console.print(
                        f"[bold red]Fehler beim Speichern in der Datenbank: {e}[/bold red]"
                    )
