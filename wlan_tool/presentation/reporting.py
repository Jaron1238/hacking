...
def generate_html_report(
    ...
    if Environment is None:
        logger.error(
            "Jinja2 ist nicht installiert. HTML-Bericht kann nicht erstellt werden."
        )
        return
    ...
    if not template_path:
        template_path = str(
            Path(__file__).parent.parent / "assets" / "templates" / "template.html"
        )
