...
class AnalysisTUI(App):
    ...
    def __init__(self, clustered_df, profiles, *args, **kwargs):
        ...
        self.cluster_ids = sorted(
            [cid for cid in clustered_df["cluster"].unique() if cid != -1]
        )
    ...
    def on_mount(self) -> None:
        ...
        if -1 in self.profiles:
            cluster_table.add_row(
                "-1", str(self.profiles[-1].get("count", 0)), "Ausreißer"
            )
    ...
    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        ...
        if event.control.id == "cluster-table":
            ...
            cluster_id = (
                int(event.cursor_row)
                if event.cursor_row < len(self.cluster_ids)
                else -1
            )
            client_table = self.query_one("#client-table", DataTable)
            client_table.clear()

            clients_in_cluster = self.clustered_df[
                self.clustered_df["cluster"] == cluster_id
            ]
