...
from .constants import Constants, ErrorCodes, ErrorMessages, get_error_message
    ...
    def validate_not_none(value: Any, field_name: str) -> None:
        ...
        if value is None:
            raise ValidationError(
                get_error_message(
                    ErrorCodes.VALIDATION_MISSING_REQUIRED_FIELD, field=field_name
                ),
    ...
    def validate_mac_address(mac: str) -> str:
        ...
        if len(mac) != Constants.MAC_ADDRESS_LENGTH:
            raise ValidationError(
                ...
                details={
                    "mac": mac,
                    "expected_length": Constants.MAC_ADDRESS_LENGTH,
                    "actual_length": len(mac),
                },
            )
        ...
        if len(ssid) > Constants.SSID_MAX_LENGTH:
            raise ValidationError(
                ...
                details={
                    "ssid": ssid,
                    "length": len(ssid),
                    "max_length": Constants.SSID_MAX_LENGTH,
                },
            )
    ...
    def validate_channel(channel: int) -> int:
        ...
        if channel not in Constants.SUPPORTED_CHANNELS:
            raise ValidationError(
                ...
                details={
                    "channel": channel,
                    "supported_channels": Constants.SUPPORTED_CHANNELS,
                },
            )
...
class FileSystemValidator(Validator):
    ...
    def validate_file_path(
        ...
        if must_exist and not resolved_path.exists():
            raise ValidationError(
                get_error_message(
                    ErrorCodes.FILE_NOT_FOUND, file_path=str(resolved_path)
                ),
...
class DataValidator(Validator):
    ...
    def validate_timestamp(
        timestamp: Union[int, float, datetime], field_name: str = "timestamp"
    ) -> float:
...
class ConfigValidator(Validator):
    ...
    def validate_config_dict(
        config: Dict[str, Any], required_keys: List[str], field_name: str = "config"
    ) -> Dict[str, Any]:
...
class BatchValidator(Validator):
    ...
    def validate_timeout(
        timeout: Union[int, float], field_name: str = "timeout"
    ) -> float:
        ...
        if not (0 < timeout <= Constants.DEFAULT_TIMEOUT * 10):  # Max 5 Minuten
            raise ValidationError(
                ...
                details={
                    ...
                    "timeout": timeout,
                    "min": 0,
                    "max": Constants.DEFAULT_TIMEOUT * 10,
                },
            )
