# data/plugins/analysis_ensemble_models.py
...
def extract_features_for_classification(state: Dict, events: list) -> Tuple[np.ndarray, np.ndarray, List[str]]:
    """
...
def create_performance_visualization(performance_metrics: List[ModelPerformance], 
                                   outdir: Path) -> None:
    """
...
def run(state: Dict, events: list, console, outdir: Path, **kwargs):
    ...
    try:
        # Features extrahieren
        X, y, client_macs = extract_features_for_classification(state, events)
        ...
        for name, model in base_models.items():
            ...
            perf = evaluate_model_performance(model, X_train, y_train, name)
        ...
        create_performance_visualization(performance_metrics, outdir)
        ...
        results = {
            ...
            'best_model': {
                ...
                'test_accuracy': test_accuracy if best_model else None
            },
