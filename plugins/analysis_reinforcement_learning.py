# data/plugins/analysis_reinforcement_learning.py
...
def train_rl_agent(environment: WiFiScanningEnvironment, agent, 
                  episodes: int = 100, console=None) -> Dict:
    """
...
def run(state: Dict, events: list, console, outdir: Path, **kwargs):
    ...
    try:
        # Umgebung erstellen
        env = WiFiScanningEnvironment(state, events)
        ...
        training_results = train_rl_agent(env, agent, episodes=50, console=console)
