...
class CaptureController:
    ...
    def run_capture(self):
        ...
        duration = self.args.duration or self.config_data.get("capture", {}).get("duration", 120)
        outdir = Path(self.args.outdir) if self.args.outdir else Path.cwd()
        pcap_out = self.args.pcap or (outdir / "capture.pcap" if self.args.project else None)
        ...
        discovery_time = self.args.discovery_time or self.config_data.get("scanning", {}).get("adaptive_scan_discovery_s", 60)

        if use_adaptive_scan and duration > discovery_time:
            self._run_adaptive_scan(monitor_iface, duration, discovery_time, db_path, pcap_out)
        else:
    ...
    def _select_interface(self) -> Optional[str]:
        ...
        if not iface:
            self.console.print("[bold cyan]Kein Interface angegeben. Suche nach geeigneten WLAN-Interfaces...[/bold cyan]")
            suitable_ifaces = pre_run_checks.find_wlan_interfaces()
            if not suitable_ifaces:
                self.console.print("[bold red]Fehler: Keine WLAN-Interfaces gefunden.[/bold red]")
                return None
            if len(suitable_ifaces) == 1:
                iface = suitable_ifaces[0]
                self.console.print(f"[green]Ein Interface gefunden: {iface}. Wird automatisch verwendet.[/green]")
            else:
                iface = Prompt.ask("Bitte wählen Sie ein Interface", choices=suitable_ifaces)
        return iface
    ...
    def _setup_monitor_mode(self, physical_iface: str) -> Optional[str]:
        ...
        """
        monitor_iface = "mon0"
        logger.info(f"Erstelle dediziertes Monitor-Interface '{monitor_iface}' aus '{physical_iface}'...")
        ...
        try:
            ...
            self.console.print("[yellow]Beende potenziell störende Netzwerkdienste...[/yellow]")
            ...
            self.console.print(f"[cyan]Deaktiviere physisches Interface '{physical_iface}'...[/cyan]")
            subprocess.run(f"sudo ip link set {physical_iface} down", shell=True, check=True, capture_output=True, text=True)

            # --- Schritt 3: Phy-Namen ermitteln ---
            # Dies sollte jetzt auf dem 'down'-Interface funktionieren.
            phy_find_cmd = f"iw dev {physical_iface} info | gawk '/wiphy/ {{printf \"phy\" $2}}'"
            result = subprocess.run(phy_find_cmd, shell=True, check=True, capture_output=True, text=True)
            phy_name = result.stdout.strip()
            if not phy_name:
                logger.error(f"Konnte den 'phy'-Namen für das Interface '{physical_iface}' nicht ermitteln.")
                return None
            logger.info(f"Physisches Gerät gefunden: {phy_name}")

            # --- Schritt 4: Monitor-Interface erstellen und aktivieren ---
            self.console.print(f"[cyan]Konfiguriere Interface '{monitor_iface}'...[/cyan]")
            setup_commands = [
            ...
            logger.info(f"Monitor-Interface '{monitor_iface}' erfolgreich erstellt und aktiviert.")
            return monitor_iface

        except subprocess.CalledProcessError as e:
            logger.error(f"Ein Befehl zur Erstellung des Monitor-Interfaces ist fehlgeschlagen.")
            ...
            subprocess.run("sudo systemctl start wpa_supplicant", shell=True, capture_output=True, text=True)
            return None
    ...
    def _run_adaptive_scan(self, monitor_iface, duration, discovery_time, db_path, pcap_out):
        """Führt den zweistufigen adaptiven Scan durch."""
        self.console.print(f"[bold cyan]Starte adaptive Erfassung: Phase 1 (Discovery) für {discovery_time}s...[/bold cyan]")
        discovery_start_ts = time.time()
        capture.sniff_with_writer(monitor_iface, discovery_time, db_path, pcap_out=pcap_out)
        
        self.console.print("[cyan]Analysiere Discovery-Daten, um Zielkanäle zu bestimmen...[/cyan]")
        with database.db_conn_ctx(db_path) as conn:
            events_for_adaptive = list(database.fetch_events(conn, start_ts=discovery_start_ts))
        ...
        if not temp_state.aps:
            logger.warning("Keine APs in der Discovery-Phase gefunden. Setze mit normalem Scan fort.")
        else:
            channel_scores = {}
            for ap in temp_state.aps.values():
                if ap.channel:
                    score = ap.count + (100 + ap.rssi_w.mean) / 10
                    channel_scores[ap.channel] = channel_scores.get(ap.channel, 0) + score
            
            if channel_scores:
                sorted_channels = sorted(channel_scores, key=channel_scores.get, reverse=True)
                top_channels = sorted_channels[:self.config_data.get("scanning", {}).get("adaptive_scan_top_n_channels", 5)]
                
                priority_channels = []
                for ch in top_channels:
                    priority_channels.extend([ch] * self.config_data.get("scanning", {}).get("adaptive_scan_priority_weight", 3))
                
                other_channels = [ch for ch in sorted_channels if ch not in top_channels]
                priority_channels.extend(other_channels)
        
        self.console.print(f"[green]Priorisierte Kanäle für die gezielte Phase: {top_channels or 'Keine gefunden'}[/green]")
        
        remaining_duration = duration - discovery_time
        self.console.print(f"[bold cyan]Starte Phase 2 (Targeted Scan) für {remaining_duration}s...[/bold cyan]")
        capture.sniff_with_writer(monitor_iface, remaining_duration, db_path, pcap_out=pcap_out, channels_override=priority_channels)
...
class AnalysisController:
    ...
    def run_analysis(self):
        ...
        self.console.print(f"[cyan]Prüfe Label-Datenbank-Schema: {self.label_db_path}[/cyan]")
        ...
        if self.args.cluster_aps is not None or self.args.export_graph or self.args.export_csv:
            clustered_ap_df = cli.print_ap_cluster_results(self.args, self.state_obj, self.console)
        ...
        if self.args.infer or self.args.html_report:
            model = joblib.load(self.args.model) if self.args.model and Path(self.args.model).exists() else None
            inference_results = analysis.score_pairs_with_recency_and_matching(self.state_obj, model)
        ...
        if self.args.export_graph or self.args.export_csv:
            self._handle_graph_export(self.state_obj, clustered_ap_df, clustered_client_df)
    ...
    def _load_state_and_events(self, state_path, db_path):
        ...
        if state_path.exists():
            try:
                self.console.print(f"[cyan]Lade gespeicherten Zustand von {state_path}...[/cyan]")
                state = joblib.load(state_path)
                if state.clients:
                    last_timestamp = max(c.last_seen for c in state.clients.values())
            except Exception as e:
                self.console.print(f"[red]Fehler beim Laden des Zustands: {e}. Baue Zustand neu auf.[/red]")
                state = None

        if state is None:
            state = WifiAnalysisState()
            
        self.console.print(f"[cyan]Lade Events aus der Datenbank (von {db_path})...[/cyan]")
        with database.db_conn_ctx(db_path) as conn:
        ...
        if not new_events and not state.clients:
            logger.warning("Keine Events in der Datenbank und kein gespeicherter Zustand. Analyse wird übersprungen.")
            return None, None

        if new_events:
            state.build_from_events(new_events, detailed_ies=self.args.detailed_ies)
        
        self.console.print(f"[green]Zustand aufgebaut/aktualisiert: {len(state.aps)} APs, {len(state.clients)} Clients, {len(state.ssid_map)} SSIDs.[/green]")
        return state, new_events

    def _save_state(self, state_obj, state_path):
        """Speichert den Zustand am korrekten Pfad."""
        try:
            self.console.print(f"[cyan]Speichere Analyse-Zustand nach {state_path}...[/cyan]")
    ...
    def _run_profiling(self):
        ...
        self.console.print("[cyan]Versuch 1: Identifizierung über FRITZ!Box TR-064...[/cyan]")
        ...
        if FritzHosts and not fritz_password:
             if self.console.input("Haben Sie ein Passwort für Ihre FRITZ!Box gesetzt? (y/n): ").lower() == 'y':
                 fritz_password = self.console.input("Bitte FRITZ!Box-Passwort eingeben: ", password=True)
        
        device_map = device_profiler.get_devices_from_fritzbox_tr064(password=fritz_password) or {}
        if device_map:
            self.console.print(f"[bold green]Erfolgreich {len(device_map)} Geräte via TR-064 gefunden![/bold green]")
        else:
            self.console.print("[yellow]TR-064-Versuch fehlgeschlagen oder keine Geräte gefunden.[/yellow]")
        
        # --- Versuch 2: Standard-UPnP (Fallback) ---
        if not device_map:
            self.console.print("\n[cyan]Versuch 2: Fallback auf aktiven ARP- und Nmap-Scan...[/cyan]")
            profile_iface = self.args.profile_iface or self.config_data.get("profiling", {}).get("interface")
            profile_net = self.args.profile_net or self.config_data.get("profiling", {}).get("network_cidr")
            if not profile_iface or not profile_net:
                ...
                if clients_with_ips:
                    ...
                    self.console.print("[yellow]Aktiver ARP-Scan konnte keine IPs ermitteln.[/yellow]")
        
        if not device_map:
            self.console.print("[red]Automatisches Profiling konnte keine Geräte identifizieren.[/red]")
        else:
            rules = device_profiler.load_device_rules()
            auto_classified = {mac: info for mac, info in device_map.items() if (info.update({'device_type': device_profiler.classify_hostname(info.get('hostname'), rules) or info.get('device_type')}), info.get('device_type'))}
            unknown = {mac: info for mac, info in device_map.items() if mac not in auto_classified}
            self.console.print(f"[green]{len(auto_classified)} Geräte automatisch via Regeln/Scan klassifiziert.[/green]")
            manual_classified = device_profiler.interactive_classify_unknowns(unknown, self.console)
            final_map = {**auto_classified, **manual_classified}
            if final_map:
                database.save_labels_from_map(final_map, self.label_db_path, self.console)
    ...
    def _run_client_clustering_if_needed(self, state_obj):
        ...
        plugins_need_clusters = self.args.run_plugins is not None and "umap_plot" in (self.args.run_plugins or self.plugins.keys())
        if plugins_need_clusters:
            return analysis.cluster_clients(state_obj, n_clusters=0, algo=self.args.cluster_algo, correlate_macs=(not self.args.no_mac_correlation))
        return None, None
        ...
        for name in plugins_to_run:
            if name in self.plugins:
                try:
                    ...
                    valid_args = {k: v for k, v in plugin_context.items() if k in sig.parameters}
                    self.plugins[name].run(**valid_args)
                except Exception as e:
                    self.console.print(f"[bold red]Fehler bei der Ausführung des Plugins '{name}': {e}[/bold red]")
